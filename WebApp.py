from flask import Flask, render_template, session, request, redirect, url_for, abort, make_response, jsonify
from Option_field_WebApp import Login
import os
import csv
import json
from search_filter import single_oxa, get_added_genomes, oxa_and_IC_multiprocessing, read_search, read_search_spec, pre_processing, pre_processing_prefilter2, read_search_pre
...
def assignspec():
    ...
    score_ct, names_ct, hits_ct, predictions = read_search_spec(reads, quick, BF_Master, ext, genus)
    ...
    if not metagenome:
        # add dynamic path
        prediction = classify(r'Training_data/' + genus + "_Training_data_spec.csv", score_ct, True)
